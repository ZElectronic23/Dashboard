...
class Utils {
  ...
  static async translateArabicToEnglish(text) {
...
class Dashboard {
  ...
  static populateUserData(userData) {
    ...
    if (elements.profileImg) elements.profileImg.src = userData.profileImg;
    if (elements.topBarAvatar) elements.topBarAvatar.src = userData.profileImg;
    if (elements.userMenuName) elements.userMenuName.textContent = userData.empName;

    // Translate name to English
    Utils.translateArabicToEnglish(userData.empName).then(translatedName => {
      const userMenuNameEn = document.getElementById("user-menu-name-en");
      if (userMenuNameEn) {
        userMenuNameEn.textContent = translatedName;
      }
    });
  }
