  ...
  static async login(username, password) {
    ...
    try {
      ...
      if (data.success && data.userData) {
        ...
        logAuditEvent("Login Success", { username: username });
        return true;
      } else {
        Utils.showError(data.message || "خطأ غير معروف");
        logAuditEvent("Login Failed", { username: username, message: data.message || "Unknown error" });
        return false;
      }
    } catch (error) {
      console.error("Login error:", error);
      Utils.showError(TRANSLATIONS[currentLang].connection_error);
      logAuditEvent("Login Error", { username: username, error: error.message });
      return false;
    } finally {
      // Reset login button
...
class EventListeners {
  static init() {
    ...
    window.logout = () => {
      Utils.logout();
      logAuditEvent("Logout", { username: Utils.getUserData()?.empName });
    };
  }
}
...
async function logAuditEvent(action, details) {
  ...
  try {
    const urlWithParams = new URL(CONFIG.API_URL);
    urlWithParams.searchParams.append("action", "logAudit");
    urlWithParams.searchParams.append("event", action);
    urlWithParams.searchParams.append("details", JSON.stringify(details));
    
    // Send a POST request to avoid caching issues with GET for logging
    const response = await fetch(urlWithParams, {
      ...
      body: JSON.stringify({ action: "logAudit", event: action, details: details }) // Added body for POST
    });
    if (!response.ok) {
    ...
    const data = await response.json();
    if (!data.success) {
      console.error("Audit log failed on server side:", data.message);
    }
  } catch (error) {
